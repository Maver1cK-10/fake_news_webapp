Fake News Detection Web App Overview
...
Project Structure FakeNewsDetection/ │ ├── app.py ├── requirements.txt ├── models/ │ ├── vectorizer.pkl │ ├── model.pkl │ ├── logistic_model.pkl │ ├── random_forest.pkl │ └── naive_bayes.pkl │ ├── templates/ │ └── result.html │ └── README.md Installation 1️) Clone the repository git clone https://github.com/yourusername/fake-news-detector.git cd fake-news-detector 2️) Create virtual environment (recommended) python -m venv venv source venv/bin/activate # macOS/Linux venv\Scripts\activate # Windows 3️) Install dependencies pip install -r requirements.txt Running the Application python app.py
...
vectorizer.transform([cleaned_text]) 3. Prediction
...
Models Used Model Accuracy (%) Logistic Regression 95.8 Random Forest 94.2 Naive Bayes 90.5 Requirements
